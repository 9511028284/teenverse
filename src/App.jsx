         ...
         <div className="p-8 max-w-6xl mx-auto">
            {tab === 'overview' && (
              <div className="grid md:grid-cols-3 gap-6 animate-fade-in">
                 <div className="bg-gradient-to-br from-indigo-600 to-violet-700 rounded-2xl p-6 text-white shadow-xl"><p className="text-indigo-100 mb-2">Total {isClient ? 'Spent' : 'Earnings'}</p><h3 className="text-4xl font-bold">₹0.00</h3></div>
            ...
            {tab === 'jobs' && (
               <div className="grid gap-6 animate-fade-in">
                  {jobs.length === 0 ? <div className="text-center py-20 text-gray-500">No jobs found.</div> : jobs.map(job => (
                    <div key={job.id} className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm hover:border-indigo-100 transition-all">
                     ...
                     <div key={app.id} className="bg-white p-6 rounded-2xl border border-gray-200 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 hover:shadow-md transition-all">
                        <div className="flex-1">
                           ...
                           <div className="flex gap-3 text-sm text-gray-500 mt-1 items-center">
                               ...
                               <span>•</span>
                               <span className={`px-2 rounded text-xs font-bold uppercase ${app.status === 'Accepted' ? 'bg-green-100 text-green-700' : app.status === 'Rejected' ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-700'}`}>{app.status}</span>
                           </div>
...
export default function TeenVerse() {
  ...
  return (
    <>
      ...
      {view === 'dashboard' && user && <Dashboard user={user} onLogout={async () => { await signOut(auth); setView('home'); showToast('Logged out successfully'); }} showToast={showToast} />}
    </>
  );
}