...
import { 
    ...
    // New icons for Client Guide
    FileText, Search, ShieldCheck, ThumbsUp 
} from 'lucide-react';
import Button from '../ui/Button'; 
import { supabase } from '../../supabase'; 
import { getEffectivePlanName } from '../../utils/subscription';

// Added isClient prop to determine the view mode
const SupportHub = ({ user, isClient, showToast }) => {
  const plan = getEffectivePlanName(user);

  // --- TICKET / CHAT STATES (Freelancer Only) ---
  ...
  // 🚀 Fetch Existing Ticket (Skipped if user is a Client or on Basic)
  useEffect(() => {
    if (isClient || plan === 'Basic') {
        setLoading(false);
        return;
    }
  ...
  }, [user.id, plan, showToast, isClient]);

  // 🚀 Submit the Initial Ticket (Freelancer Only)
  const handleSubmitTicket = async (e) => {
    e.preventDefault();
    if (isClient) return; // Safeguard
  ...
  // 🚀 Handle sending subsequent replies (Freelancer Only)
  const handleSendReply = async (e) => {
    e.preventDefault();
    if (isClient || !replyText.trim() || !ticketId) return;
    ...
    <div className="max-w-6xl mx-auto py-8 px-4 animate-fade-in">
      <div className="mb-8">
          <h2 className="text-3xl font-black text-slate-900 dark:text-white mb-2">
            {isClient ? 'Client Hub & Guides' : 'Help & Community'}
          </h2>
          {!isClient && (
      ...
      <div className="grid lg:grid-cols-12 gap-8">
        ...
        {/* ========================================== */}
        {/* LEFT COLUMN: GUIDE (CLIENT) OR TICKET (FREELANCER) */}
        {/* ========================================== */}
        <div className="lg:col-span-7 flex flex-col h-[650px] bg-white dark:bg-[#0F172A] border border-slate-200 dark:border-white/10 rounded-[2rem] shadow-xl overflow-hidden relative">
            ...
            {isClient ? (
                // 🏢 CLIENT VIEW: PLATFORM GUIDE
                <div className="flex flex-col h-full">
                    <div className="p-6 border-b border-slate-200 dark:border-white/10 bg-indigo-50/50 dark:bg-indigo-900/10">
                        <h3 className="font-black text-xl text-slate-900 dark:text-white">How TeenVerseHub Works</h3>
                        <p className="text-sm text-slate-500 mt-1">Your step-by-step guide to hiring safely and effectively.</p>
                    </div>
                    
                    <div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar bg-slate-50/30 dark:bg-transparent">
                        
                        {/* Step 1 */}
                        <div className="flex gap-4">
                            <div className="flex-shrink-0 w-12 h-12 bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-2xl flex items-center justify-center">
                                <FileText size={24} />
                            </div>
                            <div>
                                <h4 className="text-lg font-bold text-slate-900 dark:text-white mb-1">1. Post Your Gig</h4>
                                <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed">
                                    Navigate to your Dashboard and create a new job posting. Be clear about your requirements, timeline, and budget to attract the best talent.
                                </p>
                            </div>
                        </div>
                        ...
                        {/* Step 2 */}
                        <div className="flex gap-4">
                            <div className="flex-shrink-0 w-12 h-12 bg-purple-100 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400 rounded-2xl flex items-center justify-center">
                                <Search size={24} />
                            </div>
                            <div>
                                <h4 className="text-lg font-bold text-slate-900 dark:text-white mb-1">2. Review Proposals</h4>
                                <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed">
                                    Freelancers will apply to your gig. Review their applications, check their portfolios, and look for Academy Badges to verify their skills.
                                </p>
                            </div>
                        </div>
                        ...
                        {/* Step 3 */}
                        <div className="flex gap-4">
                            <div className="flex-shrink-0 w-12 h-12 bg-amber-100 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 rounded-2xl flex items-center justify-center">
                                <ShieldCheck size={24} />
                            </div>
                            <div>
                                <h4 className="text-lg font-bold text-slate-900 dark:text-white mb-1">3. Secure Escrow Payment</h4>
                                <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed">
                                    When you hire someone, your funds are placed in a neutral escrow account. The freelancer knows they will be paid, and your money is safe until the work is done.
                                </p>
                            </div>
                        </div>
                        ...
                        {/* Step 4 */}
                        <div className="flex gap-4">
                            <div className="flex-shrink-0 w-12 h-12 bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 rounded-2xl flex items-center justify-center">
                                <ThumbsUp size={24} />
                            </div>
                            <div>
                                <h4 className="text-lg font-bold text-slate-900 dark:text-white mb-1">4. Approve & Release</h4>
                                <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed">
                                    Once the freelancer delivers the project and you are fully satisfied, you approve the work to release the funds directly to them.
                                </p>
                            </div>
                        </div>
                ...
                // 🧑‍💻 FREELANCER VIEW: FOUNDER HELP DESK
                <>
        ...
        <div className="lg:col-span-5 space-y-6">
            ...
            {/* Standard Email Card (Visible to Both) */}
            <div className="bg-white dark:bg-[#0F172A] border border-slate-200 dark:border-white/10 rounded-3xl p-6 shadow-sm">
                <h3 className="text-lg font-bold mb-4 flex items-center gap-2 text-slate-900 dark:text-white">
                    <ShieldAlert size={20}/> 
                    {isClient ? 'Need Help?' : 'Standard Support'}
                </h3>
                ...
                {isClient && (
                    <p className="text-xs text-slate-500 mt-4 leading-relaxed">
                        If you experience any issues with payments, escrow, or unresponsive freelancers, please email us directly. Our team reviews client claims within 24 hours.
                    </p>
                )}
            </div>

            {/* FREELANCER ONLY: VIP WhatsApp & Discord */}
            {!isClient && (
                <>
                    {/* VIP WhatsApp Card */}
