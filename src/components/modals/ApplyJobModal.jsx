...
const ApplyJobModal = ({ onClose, onSubmit, job, user, currentEnergy }) => {
  ...
  const platformFee = numericBidAmount * commissionRate;
  const payoutAmount = Math.max(0, numericBidAmount - platformFee);
    ...
    <Modal title={`Apply: ${job.title}`} onClose={onClose}>
      <form onSubmit={handleSubmit} className="space-y-6 relative z-10">
        ...
        <div className="bg-[#09090b] p-6 rounded-[24px] border border-white/10 space-y-5">
            ...
            <div className="grid grid-cols-3 items-center gap-2 text-[10px] font-mono border-t border-dashed border-white/10 pt-4">
                <div className="text-center">
                    <span className="text-gray-500 block mb-1">BID AMOUNT</span>
                    <span className="text-white font-bold text-sm">₹{numericBidAmount.toLocaleString()}</span>
                </div>
                <div className="text-center">
