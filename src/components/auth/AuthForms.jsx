...
import { Mail, Lock, Eye, EyeOff, Loader2, User, Briefcase, Check, Key, Globe, Gift, ArrowRight, ArrowLeft } from 'lucide-react';
import { Turnstile } from '@marsidev/react-turnstile';
import { StepIndicator, SocialButton, BackButton } from './AuthUI';

// --- ICONS ---
const GoogleIcon = memo(() => (<svg viewBox="0 0 24 24" className="w-5 h-5"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/></svg>));
const GithubIcon = memo(() => (<svg viewBox="0 0 24 24" className="w-5 h-5 fill-current"><path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"/></svg>));

// ⚡ OPTIMIZATION: Hardware acceleration, reduced motion hook, and removed heavy CSS filters
const Floating3DIcon = memo(({ src, className, delay = 0 }) => {
  ...
  return (
    <motion.img 
      ...
      loading="lazy"
      decoding="async"
      // Disable loop if device prefers reduced motion/battery saver
      animate={prefersReducedMotion ? {} : { y: [0, -12, 0] }}
      transition={{ duration: 4, repeat: Infinity, ease: "easeInOut", delay }}
      // Force GPU layer creation to stop main-thread layout thrashing
...
export const LoginView = memo(({ state, actions, refs }) => {
  ...
  return (
    <motion.div key="login" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="relative">
      ...
      {/* 🛡️ CLOUDFLARE TURNSTILE ADDED HERE */}
      {state.CLOUDFLARE_SITE_KEY && (
          <div className="mt-6 flex justify-center py-2 bg-slate-100 dark:bg-black/20 rounded-2xl border border-slate-200 dark:border-white/5 relative z-10">
              <Turnstile 
                 ref={refs.turnstileRef} 
                 siteKey={state.CLOUDFLARE_SITE_KEY} 
                 onSuccess={actions.setCaptchaToken} 
                 onExpire={() => actions.setCaptchaToken(null)} 
                 theme="auto" 
              />
          </div>
      )}
    ...
    <motion.div key="signup" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="h-full flex flex-col relative">
            ...
            <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} className="space-y-8 relative">
                ...
                <div className="grid gap-4">
                    {['freelancer', 'client'].map((r) => (
                        <button key={r} onClick={() => actions.updateField('role', r)} className={`p-6 rounded-3xl border transition-all text-left relative overflow-hidden group hover:scale-[1.02] active:scale-[0.98] duration-300 ${formData.role === r ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-500/10 shadow-lg' : 'border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 hover:border-indigo-300 dark:hover:border-white/20 shadow-sm dark:shadow-none'}`}>
                            <div className="absolute inset-0 bg-gradient-to-r from-transparent via-indigo-500/5 dark:via-white/5 to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-1000"/>
                            <div className="flex justify-between items-start mb-4 relative z-10">
                                <div className={`p-4 rounded-2xl ${formData.role === r ? 'bg-indigo-600 text-white shadow-md' : 'bg-slate-100 dark:bg-gray-800 text-slate-500 dark:text-gray-400'}`}>{r === 'freelancer' ? <User size={28}/> : <Briefcase size={28}/>}</div>
                                {formData.role === r && <div className="bg-indigo-600 text-white p-1.5 rounded-full shadow-md"><Check size={16}/></div>}
                            </div>
        ...
        <div className="mt-auto pt-8 border-t border-slate-200 dark:border-white/10 flex justify-end">
            {step < 4 ? 
              ...
              : 
              <button onClick={actions.handleFinalSubmit} disabled={state.loading} className="bg-gradient-to-r from-indigo-600 to-purple-600 text-white hover:brightness-110 px-10 py-4 rounded-2xl font-black shadow-lg transition-all transform active:scale-95 flex items-center gap-3 uppercase tracking-widest text-sm">
      ...
      {!state.showResetVerify ? (
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="relative">
              ...
              <div className="relative mb-6 group">
                  ...
                  <input className="w-full bg-white dark:bg-black/40 border border-slate-200 dark:border-gray-700 rounded-2xl py-5 pl-14 pr-4 text-center text-3xl tracking-[0.5em] font-mono text-slate-900 dark:text-white focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/20 outline-none transition-all shadow-sm" placeholder="000000" maxLength={6} value={state.resetOtp} onChange={(e) => actions.setResetOtp(e.target.value)} />
              </div>
              
              <button onClick={actions.handleVerifyResetOTP} disabled={state.loading} className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-black py-4 rounded-2xl shadow-lg flex justify-center items-center transform active:scale-95 transition-all uppercase tracking-widest text-sm">
  ...
  return (
    <motion.div key="update-pw" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="relative">
       ...
       <form onSubmit={actions.handleUpdatePassword} className="space-y-6">
         ...
         <button type="submit" disabled={state.loading} className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-black py-4 rounded-2xl shadow-lg transition-all transform active:scale-95 uppercase tracking-widest text-sm mt-4">
