...
const Dashboard = ({ user, setUser, onLogout, showToast, darkMode, toggleTheme }) => {
  ...
  // --- FIX: Define SAFE_QUIZZES to prevent crash ---
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-1 custom-scrollbar">
           ...
           {!isClient && (
             <>
               ...
               <button onClick={() => {setTab('battles'); setMenuOpen(false);}} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium ${tab === 'battles' ? 'bg-indigo-50 dark:bg-indigo-900/20 text-indigo-700 dark:text-indigo-300' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800'}`}><Swords size={18}/> Battles</button>
           ...
           <button onClick={() => {setTab('records'); setMenuOpen(false);}} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium ${tab === 'records' ? 'bg-indigo-50 dark:bg-indigo-900/20 text-indigo-700 dark:text-indigo-300' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800'}`}><FileCheck size={18}/> Records & Compliance</button>
                        ...
                        <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                           ...
                           {applications.map(app => (
                              <tr key={app.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                                 <td className="p-4 font-mono text-xs text-gray-500 dark:text-gray-400">#{String(app.job_id || "").slice(0,8)}</td>
