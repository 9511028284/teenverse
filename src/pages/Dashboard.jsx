...
import { 
  ...
  Award, Sun, Moon, Bell, PlusCircle, CreditCard, Lock, Receipt, X, Package, Share2, Swords, Crown, ShieldCheck,
  Maximize2, Minimize2, User, CheckCircle, Download // <--- Added CheckCircle & Download here
} from 'lucide-react';
import html2canvas from 'html2canvas'; // <--- Added for Downloading Card
...
const Dashboard = ({ user, setUser, onLogout, showToast, darkMode, toggleTheme }) => {
  ...
  // --- 2. FIXED: ADDED MISSING AI GENERATE FUNCTION ---
  const handleAiGenerate = () => {
    setIsAiLoading(true);
    // Simulate AI generation
    setTimeout(() => {
      setRawPortfolioText("Here is a draft of your project description generated by AI based on your skills...");
      setIsAiLoading(false);
      showToast("AI Content Generated!", "success");
    }, 1500);
  };

  // --- 3. FIXED: ADDED MISSING DOWNLOAD FUNCTION ---
  const handleDownloadCard = async () => {
    if (cardRef.current) {
      try {
        ...
        link.href = image;
        link.download = `Profile_${user.name?.replace(/\s+/g, '_') || 'User'}.png`;
        link.click();
  ...
  const handleCreateService = async (e) => {
    ...
    if (error) { showToast(error.message, 'error'); } else { showToast('Gig Created!'); setModal(null); setServices([serviceData, ...services]); }
  };

  const handleDeleteService = async (id) => {
    if(!window.confirm("Delete gig?")) return;
  ...
  const handleApplyJob = async (e) => {
    ...
    if (!isClient && selectedJob) {
      ...
      if (!unlockedSkills.includes(jobCategory)) {
        showToast(`Locked! Pass quiz first.`, "error");
    ...
    if (error) { showToast(error.message, 'error'); } else { await supabase.from('notifications').insert([{ user_id: selectedJob.client_id, message: `New application: ${selectedJob.title}` }]); showToast('Applied!'); setModal(null); }
  };
  ...
  const processPayment = async () => {
    ...
    if (error) { showToast(error.message, 'error'); } else { await supabase.from('notifications').insert([{ user_id: freelancerId, message: `💰 Payment received for Job #${String(appId).slice(0,6)}` }]); showToast("Payment Successful!", "success"); setApplications(apps => apps.map(a => a.id === appId ? { ...a, status: 'Paid' } : a)); setPaymentModal(null); }
  };
  ...
  const SidebarItem = ({ id, icon: Icon, label, color }) => (
    <button onClick={() => {setTab(id); setMenuOpen(false);}} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all duration-200 ${tab === id ? 'bg-indigo-50 dark:bg-indigo-900/20 text-indigo-700 dark:text-indigo-300 shadow-sm translate-x-1' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800 hover:text-gray-900 dark:hover:text-gray-200'}`}>
  ...
  return (
    <div className="flex h-screen bg-gray-50 dark:bg-[#0B1120] transition-colors duration-300 font-sans overflow-hidden">
      ...
      {/* Mobile Backdrop */}
      {menuOpen && ( <div className="fixed inset-0 bg-black/60 z-40 md:hidden backdrop-blur-sm transition-opacity" onClick={() => setMenuOpen(false)} /> )}

      {/* Sidebar */}
      <aside className={`fixed md:static inset-y-0 left-0 z-50 bg-white dark:bg-[#0F172A]/95 backdrop-blur-xl border-r border-gray-200 dark:border-gray-800 transform transition-all duration-300 ease-in-out ${menuOpen ? 'translate-x-0 w-64' : '-translate-x-full md:translate-x-0'} ${zenMode ? 'md:w-20' : 'md:w-72'} flex flex-col h-full shadow-2xl md:shadow-none`}>
        <div className={`p-6 border-b border-gray-100 dark:border-gray-800 flex ${zenMode ? 'justify-center' : 'justify-between'} items-center flex-shrink-0`}>
          <div className="flex items-center gap-3 font-black text-xl text-gray-900 dark:text-white overflow-hidden">
            <div className={`flex-shrink-0 w-10 h-10 rounded-xl bg-gradient-to-br ${COLORS?.primary || 'from-indigo-600 to-purple-600'} flex items-center justify-center text-white shadow-lg shadow-indigo-500/30`}><Rocket size={20} /></div>
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-gray-50 dark:bg-[#0B1120] relative">
         <header className="bg-white/80 dark:bg-[#0F172A]/80 backdrop-blur-md sticky top-0 z-30 border-b border-gray-200 dark:border-gray-800 px-4 md:px-8 py-4 flex justify-between items-center">
            <div className="flex items-center gap-3 md:gap-4">
               ...
               <button onClick={() => setZenMode(!zenMode)} className="hidden md:flex p-2 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg text-gray-500 dark:text-gray-400">{zenMode ? <Maximize2 size={20}/> : <Minimize2 size={20}/>}</button>
            ...
            <div className="flex items-center gap-2 md:gap-4">
               <button onClick={toggleTheme} className="p-2 rounded-full hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-600 dark:text-gray-300">{darkMode ? <Sun size={20} className="text-amber-400" /> : <Moon size={20} className="text-indigo-600" />}</button>
         ...
         <div className="flex-1 overflow-y-auto p-4 md:p-8 scroll-smooth custom-scrollbar">
            ...
            {/* --- 4. UPDATED: Pass handleAiGenerate here --- */}
            ...
            {/* --- 5. UPDATED: Pass Ref and Download Function here --- */}
        ...
        <Modal title="Post a Job" onClose={() => setModal(null)}>
          <form onSubmit={handlePostJob} className="space-y-4">
            <div className="space-y-1">
               ...
               <input name="title" type="text" placeholder="e.g. React Dev" className="w-full p-3 bg-gray-50 dark:bg-[#0F172A] border rounded-xl dark:border-gray-700 dark:text-white outline-none" required />
            </div>
            <div className="grid grid-cols-2 gap-4">
               <div className="space-y-1"><label className="text-xs font-bold text-gray-500 uppercase">Budget (₹)</label><input name="budget" type="number" className="w-full p-3 bg-gray-50 dark:bg-[#0F172A] border rounded-xl dark:border-gray-700 dark:text-white outline-none" required /></div>
               <div className="space-y-1"><label className="text-xs font-bold text-gray-500 uppercase">Duration</label><input name="duration" type="text" className="w-full p-3 bg-gray-50 dark:bg-[#0F172A] border rounded-xl dark:border-gray-700 dark:text-white outline-none" required /></div>
            </div>
            <div className="space-y-1">
