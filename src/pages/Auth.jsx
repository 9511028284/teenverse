...
const Auth = ({ setView, onLogin, onSignUpSuccess }) => {
  ...
  // 🆕 OTP PASSWORD RESET STATES (ADDED)
  ...
  // 📧 🆕 STEP 1: SEND RESET OTP (Replaced resetPasswordForEmail)
  const handleForgotPassword = async (e) => {
    ...
    setLoading(true);
    try {
      // ✅ Call Edge Function 'request-reset' (Action: Send)
  ...
  // 🔐 🆕 STEP 2: VERIFY RESET OTP (THE FIX)
  const handleVerifyResetOTP = async () => {
    setLoading(true);
    try {
        // ✅ Call Edge Function 'request-reset' (Action: Verify)
        ...
        // ✅ LOGIN FIX: Check if token is hash or legacy
        let loginError;
        ...
        if (data.type === 'hash') {
            ...
            const res = await supabase.auth.verifyOtp({
                ...
                type: 'email' // Important: type is 'email' for hashed recovery tokens
            });
