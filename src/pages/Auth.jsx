...
// LEGAL: Users created BEFORE this date bypass email verification
const LEGACY_CUTOFF_DATE = new Date('2025-12-16T00:00:00');

const styles =   @keyframes gradient-xy {   0%, 100% { background-position: 0% 50%; }   50% { background-position: 100% 50%; }   }   .animate-gradient {   background-size: 200% 200%;   animation: gradient-xy 15s ease infinite;   }   .glass-panel {   background: rgba(22, 27, 44, 0.7);   backdrop-filter: blur(20px);   -webkit-backdrop-filter: blur(20px);   border: 1px solid rgba(255, 255, 255, 0.08);   }   .custom-scrollbar::-webkit-scrollbar { width: 6px; }   .custom-scrollbar::-webkit-scrollbar-track { background: rgba(255,255,255,0.05); }   .custom-scrollbar::-webkit-scrollbar-thumb { background: rgba(99,102,241,0.5); border-radius: 10px; }  ;
...
try {  
  if (viewMode === 'login') {  
    ...
    if (formData.role === 'freelancer' && isMinor) {  
       ...
       localStorage.setItem('last_otp_sent', Date.now().toString()); // Set timestamp  
       setShowVerify(true);  
       setLoading(false);  
       return;   
    }  
...
e.preventDefault();
if (!parentAgreed) return alert("Parent/Guardian must explicitly consent to the terms by checking the box.");
...
// --- SECURE FILE UPLOAD (COMPRESSED + PRIVATE) ---  
let filePath = ""; // Changed from fileUrl to filePath for privacy  
if (file) {  
  try {  
    // 1. Compress  
    ...
    // 3. Store PATH only (Secure) - Do NOT generate Public URL  
    filePath = uploadData.path;   

  } catch (uploadErr) {  
    console.error("Upload failed", uploadErr);  
    // Fail-safe: if compression fails, try original  
