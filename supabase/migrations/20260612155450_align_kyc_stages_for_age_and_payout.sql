-- Align KYC stages with the split age-verification and payout-KYC flows.
...
begin
  ...
  set is_bank_linked = true,
      kyc_status = case
        when coalesce(kyc_status, '') in ('payout_kyc_pending', 'verified', 'approved', 'fully_verified') then 'payout_ready'
        else coalesce(kyc_status, 'none')
      end
...
begin
  ...
  if not (
    coalesce(v_is_kyc_verified, false)
    or coalesce(v_kyc_status, '') in (
      'approved',
      'verified',
      'age_verified',
      'payout_kyc_pending',
    ...
    raise exception 'Age verification is required before applying.';
  end if;
