-- Temporary bypass for freelancer KYC on job applications.
-- Reversal: change public.is_freelancer_kyc_application_bypass_enabled() to return false,
-- then deploy the migration/function update. This does not update KYC status or grant badges.
...
comment on function public.is_freelancer_kyc_application_bypass_enabled() is
  'TEMP: permits unverified freelancers to apply for jobs without changing KYC state or awarding identity badges.';
...
begin
  ...
  if not public.is_freelancer_kyc_application_bypass_enabled()
     and not (coalesce(v_is_kyc_verified, false) or coalesce(v_kyc_status, '') in ('approved', 'verified')) then
